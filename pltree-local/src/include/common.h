...
namespace plt {
    ...
    public:
        ...
        innerSlot():minkey(0),maxkey(0),slope(0),size(0),levelnum(0),v(0),isLeaf(0){}


        void setSlot(KeyType _minkey,KeyType _maxkey,uintptr_t _ptr,float _slope,uint32_t _size,uint8_t _v,uint8_t _isLeaf){
            minkey=_minkey;
            maxkey=_maxkey;
    ...
    public:
        ...
        inline void addKV(const SegmentMessage<KeyType> &seg_msg,std::vector<innerSlot<KeyType>> &segs){
                uint32_t fanout=seg_msg.size;
                for(int i=0;i<fanout;++i){
                    //memcpy(&slots[i],&segs[seg_msg.offset+i],sizeof(innerSlot<KeyType>));
                   slots[i]=segs[seg_msg.offset+i];
                }
         }
        ...
        inline innerSlot<KeyType>* find(const KeyType key,innerSlot<KeyType>* segInf,size_t max_error){
                SearchBound bound = GetSearchBound(key,segInf, max_error);
                //auto it = std::lower_bound(node + bound.begin, node + bound.end,SegmentInf<KeyType>{key,0}, [](const SegmentInf<KeyType> &a,const SegmentInf<KeyType> &b){return a.maxkey<b.maxkey;});
                for(int i=bound.begin;i<=bound.end;++i){
                    if(slots[i].maxkey>=key)
                        return &slots[i];
                }
                //size_t pos = it - node;
                // return node[pos].node;
            }
        inline SearchBound GetSearchBound(const KeyType key,innerSlot<KeyType>* segInf,const size_t max_error)  {
                size_t estimate = 0;
                if (key >= segInf->minkey)
                    estimate = segInf->slope * (key - segInf->minkey);
                else return {0, 0};

                size_t begin = 0, end = 0;
                // `end` is exclusive.
                if ( slots[estimate].maxkey< key) {
                    begin = (estimate + 1 > segInf->size) ? segInf->size-1: estimate + 1;
                    end = (estimate + max_error + 1 > segInf->size) ? segInf->size-1 : (estimate + max_error + 1);
                } else {
...
/*        innerSlot<KeyType> * findSlot(KeyType key){
    ...
    struct BufferMetaPtr{
        uintptr_t ptr:48;
        uintptr_t levelNum:16;
    };
    template<typename KeyType, typename ValueType>
    struct Block{
