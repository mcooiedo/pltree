...
namespace plt {
    ...
    public:
        ...
        Layer(const size_t &level, uint64_t &fpPtr,  EntryFp * lastFPEntrys,innerSlot<KeyType> *_slot) : level(level), nextlevel(nullptr) {
        ...
        void rehash(uint32_t targetLevel, uint32_t entryIdx, uint32_t bucketIdx, uint32_t rehash, KV **kvs,
            ...
            for (uint32_t i = 0; i < rehash; ++i) {
                ...
                newEntryIdx = rehashEntryIdx(newEntryIdx,targetLevel);//newEntryIdx >> (INIT_ENTRY_BIT + FANOUT * (targetLevel - 1));
    ...
    public:
        ...
        ~Segment() {
         ...
         OpStatus upsert(const KeyType &key, const ValueType &value, const int &idx,  blockMeta *bm,innerSlot<KeyType> *_slot) {
            uint8_t kvPerBlock = (BLOCK_SIZE - sizeof(BlockHeader))/sizeof(KV);
...
/*        inline bool find(KeyType key,ValueType &value,SegInfType * seginf){
            ...
            while (fp_entrys!=NULL){
                ...
                for (int i = BUCKET_PER_ENTRY-1; i >= 0; i--) {
                    ...
                    while (mask != 0) {
                        ...
                        auto ret = findInLayer(key, value,  position, reinterpret_cast<EntryType *>(fp_entrys[entryIdex].entryPtr));
                        if (ret) {
                            if (value != DELETED)
                                return true;
                            else
        ...
        void updateResults(const KeyType &lower_bound, std::vector<KV> &kvs, const int &num, const int &deleted,
         ...
         int
        rangeScan(const KeyType &key, const int &num, int &deleted, std::map<KeyType, ValueType> &results,  int idx,const int &blocknum) {
             if(idx==-1)
                 return 0;
         ...
         int
        scanAtBlock(const int &idx, const KeyType &key, const int &num,int &deleted, std::map<KeyType, ValueType> &results, const int &blocknum) {
            std::vector<KV> kvs;
            int blockSize = (BLOCK_SIZE - sizeof(BlockHeader)) / sizeof(KV);
            if (blocknum > 0) {
                for (int i = 0; i < blockSize; ++i) {
        ...
        inline void newDir(DirType *&dir, const int &idx, const size_t &level,blockMeta *bm,innerSlot<KeyType> *_slot){
            if (dir == nullptr) {
                uint64_t fpPtr=0;
        ...
        inline void AddKV(const SegmentMessage<KeyType> &seg_msg, const std::vector<KV> &kvs,innerSlot<KeyType>* &segInf,uint8_t version) {
            //
            int kvsPerBlock = (BLOCK_SIZE - sizeof(BlockHeader))/sizeof(KV);
            ...
            innerSlot<KeyType>  *slot=new innerSlot<KeyType>();
            slot->setSlot(minKey,maxKey,(uintptr_t)this,slope_,blockNum,0,leafset);
           // segInf=new innerSlot<KeyType>(maxKey,minKey,blockNum,0,slope_, (uintptr_t) this);1111
            slot->setBufferPtr(reinterpret_cast<uint64_t>(metas));
            segInf=slot;
...
/*                my_alloc::allocMem(metas, blockNum);
                ...
                for (int i = start; i < end; i++) {
                    /*if(kvs[i].first==5067167772873796717||5067167772873796717==kvs[i].first){
                        int y=0;
                    }*/
