...
Alert<T,P> *generate_index()
{
    Alert<T,P> *index = nullptr;
    auto start_time = std::chrono::high_resolution_clock::now();
    if (index_type == "apex")
    {
        //
//        my_alloc::MemPoolAllocator alloc=my_alloc::MemPoolAllocator(layout_name,pool_name,pool_size);
//        void* index_pm=alloc.GetRoot(sizeof(Alert<T,P>));
//        if(!alloc.isRecover()){
//            new(index_pm)Alert<T,P>(alloc);
//        }
//       // index=index_pm;
//        index = reinterpret_cast<Alert<T,P> *>(index_pm);

    /*    my_alloc::PMAllocator alloc=my_alloc::PMAllocator(layout_name,pool_name,pool_size);
        void * index_pm ;//= alloc.GetRoot(sizeof(Alert<T,P>));
        if(!alloc.isRecover()){
          index_pm = alloc.GetRoot(sizeof(Alert<T,P>));
            //alloc.PMZAllocAt(&index_pm,sizeof(Alert<T,P>));
             new(index)Alert<T,P>(alloc);
        }
        index = reinterpret_cast<Alert<T,P> *>(index_pm);
    */
    }
...
void GeneralBench(Alert<T,P> *index, Range *rarray, int thread_num, void (*test_func)(Alert<T,P> *, struct Range *))
{
...
void Run()
{
   ...
   // Alert<T, P> *index = generate_index<T, P>();
    Alert<T, P> *index;
   ...
   // exit(-1);
    ...
    new(index_pm)Alert<T,P>(CHECK);
    Alloc::Persist(&index_pm,sizeof(Alert<T,P>));
    // index=index_pm;
    index = reinterpret_cast<Alert<T,P> *>(index_pm);
    char *rammem;
#ifdef LARGE_DRAM
...
int main(int argc, char *argv[]) {
   ...
   if (keys_type == "double")
    ...
    /* /*
    my_alloc::PMAllocator alloc=my_alloc::PMAllocator(layout_name,pool_name,pool_size);
    plt::Alert<uint64_t,uint64_t> tree=plt::Alert<uint64_t,uint64_t>(alloc);
    uint64_t key=1;
    uint64_t value=2;
    //tree.Insert(key,value);
    std::cout << "Hello, World!" << std::endl;
    uint64_t avalue=0;
    //tree.Find(key,avalue);
