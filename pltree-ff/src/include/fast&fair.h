...
using _payload_t=char*;
constexpr static uint32_t BTREE_PAGESIZE = 256;

class page;
class btree;
//template<typename _key_t, typename ValueType, typename Alloc=my_alloc::MemPoolAllocator>
struct Record {
    _key_t key;
    char * val; 
...
///template<typename _key_t, typename pay, typename Alloc=my_alloc::MemPoolAllocator>
class btree {
    private:
    ...
    //int range_query_by_size (_key_t lower_bound, int toscan, std::map<_key_t, _payload_t>& answers);
...
class page{
    ...
    public:
        ...
        page(page* left, _key_t key, page* right, uint32_t level = 0):hdr() {
            ...
            Alloc::do_flush_with_double_fence((char*)this, sizeof(page));
        }

        void *operator new(size_t size) {
            void * ret =Alloc::PMZAlloc(size);// galc->malloc(size);
            return ret;
        }
        ...
        inline bool remove_key(_key_t key) {
            ...
            for(i = 0; records[i].val != NULL; ++i) {
                ...
                if(shift) {
                    ...
                    if(do_flush) {
                        Alloc::do_flush_with_double_fence((char *)records_ptr, CACHELINE_SIZE);
                    }
                }
            }
                ...
                if(this == root) {
                    if(hdr.level > 0) {
                        if(num_entries_before == 1 && !hdr.sibling_ptr) {
                            ...
                            Alloc::do_flush_with_double_fence(&(bt->root), 8);
            ...
            if(total_num_entries > cardinality - 1) { // Redistribution
                ...
                if(num_entries < left_num_entries) { // left -> right
                    if(hdr.leftmost_ptr == NULL){
                        ...
                        Alloc::do_flush_with_double_fence((char *)&(left_sibling->records[m].val), sizeof(char *));

                        left_sibling->hdr.last_index = m - 1;
                        Alloc::do_flush_with_double_fence((char *)&(left_sibling->hdr.last_index), sizeof(int16_t));
                        ...
                        Alloc::do_flush_with_double_fence((char *)&(hdr.leftmost_ptr), sizeof(page *));
                    ...
                    hdr.is_deleted = 1;
                    Alloc::do_flush_with_double_fence((char *)&(hdr.is_deleted), sizeof(uint8_t));
                    ...
                    if(hdr.leftmost_ptr == NULL){
                        ...
                        Alloc::do_flush_with_double_fence((char *)(new_sibling), sizeof(page));

                        left_sibling->hdr.sibling_ptr =(new_sibling);
                        Alloc::do_flush_with_double_fence(&(left_sibling->hdr), sizeof(header));
                ...
                Alloc::do_flush_with_double_fence((char *)&(left_sibling->hdr.sibling_ptr), sizeof(page *));
            }
        ...
        inline void 
            insert_key(_key_t key, char * ptr, int *num_entries, bool flush = true,
                ...
                if(*num_entries == 0) {  // this page is empty
                    ...
                    if(flush) {
                        Alloc::do_flush_with_double_fence((char*) this, CACHELINE_SIZE);
                    }
                }
                else {
                    ...
                    if(flush) {
                        if((uint64_t)&(records[*num_entries+1].val) % CACHELINE_SIZE == 0) 
                            Alloc::do_flush_with_double_fence((char*)&(records[*num_entries+1].val), sizeof(char*));
                    }
                    ...
                    for(i = *num_entries - 1; i >= 0; i--) {
                        if(key < records[i].key ) {
                            ...
                            if(flush)
                                Alloc::do_flush_with_double_fence((char*)&records[i+1],sizeof(Record));
                            inserted = 1;
                            break;
                        }
                    }
                    if(inserted==0){
                        ...
                        if(flush)
                            Alloc::do_flush_with_double_fence((char*) &records[0], sizeof(Record));
                    }
                }
        ...
        page *store (btree* bt, char * left, _key_t key, char * right,
             bool flush, page *invalid_sibling = NULL) {
                ...
                if(num_entries < cardinality - 1) {
                    ...
                    Alloc::do_flush_with_double_fence((char *)sibling, sizeof(page));

                    hdr.sibling_ptr = (page *)(sibling);
                    Alloc::do_flush_with_double_fence((char*) &hdr, sizeof(hdr));
                    ...
                    if(IS_FORWARD(hdr.switch_counter))
                        ...
                        ++hdr.switch_counter;
                    records[m].val = NULL;
                    Alloc::do_flush_with_double_fence((char*) &records[m], sizeof(Record));

                    hdr.last_index = m - 1;
                    Alloc::do_flush_with_double_fence((char *)&(hdr.last_index), sizeof(int16_t));
        ...
        bool update_value(_key_t key, _payload_t value) {
                ...
                if(key == records[pos].key) {
                        ...
                        Alloc::do_flush_with_double_fence((char *)&(records[pos]), sizeof(Record));
...
/*    int range_query (_key_t lower_bound, int toScan, std::map<_key_t, _payload_t>& answers) {
        int i = 0;
        while(records[i].val != NULL&&toScan-i>0) {
            if (records[i].key >= lower_bound ) {
                answers.insert(std::make_pair<_key_t, _payload_t>(std::move(records[i].key), std::move((_payload_t)records[i].val)));
            }
            ++i;
        }
        return toScan-i;
