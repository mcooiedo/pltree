    ...
    public:
        ...
        upsert(const KeyType &key, const ValueType &value,  innerSlot<KeyType> *_slot) {
            ...
            if (buffer != 0) {
                ...
                overflow_tree = new btree(&ptr, false);
                ret1 = overflow_tree->find(key, v);
                if (ret1) {
                    overflow_tree->upsert(key, value, 0);
                    meta->release_lock();
                ...
                if (pos < kvPerBlock) {
                    ...
                    auto ptr=(void*)buffer;
                    overflow_tree = new btree(&ptr, true);
                    overflow_tree->upsert(key, value, 0);
                    blocks[idx].head.ptr = reinterpret_cast<uint64_t>(ptr);
                    meta->setLayerPtr(reinterpret_cast<uint64_t>(ptr));
                    Alloc::Persist(&blocks[idx].kv[pos], sizeof(KV));
                }
            }
        ...
        OpStatus
        findInBM(const KeyType &key, ValueType &value, const int &idx, blockMeta *bm, innerSlot<KeyType> *_slot) {
            auto meta = bm + idx;
            ...
            if (unlikely(buffer != 0)) {
                auto ptr = (void *) buffer;
                btree *overflow_tree = new btree(&ptr, false);
                ret1 = overflow_tree->find(key, value);
                if (meta->check_version_change(old_version) || meta->check_size_change(pos))
                    return OpStatus::Retry;
            }
            return ret1 ? OpStatus::Find : OpStatus::NotFound;
        ...
        OpStatus find(const KeyType &key, ValueType &value, const int &idx, blockMeta *bm, innerSlot<KeyType> *_slot) {
            ...
            return findInBM(key, value, idx, bm, _slot);
...
/*                my_alloc::allocMem(metas, blockNum);
                ...
                for (int i = start; i < end; i++) {
                    ...
                    if (meta->num < kvsPerBlock) {
                        ...
                        void * ptr;
                        btree *overflow_tree = new btree(&ptr, !blockHead->ptr);
                        overflow_tree->insert((uint64_t)kvs[i].first, (char*)kvs[i].second);
                        if (meta->getLayerPtr() == 0) {
                            blockHead->ptr= reinterpret_cast<uint64_t>(ptr);
                            meta->setLayerPtr(blockHead->ptr);
                        }
                        ////my_alloc::statistic.maxoverflow++;
                        //migrate(idx,metas,_slot);
                        //blockHead->num=0;
