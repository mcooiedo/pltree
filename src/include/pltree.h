...
#ifndef PALERT_PALERT_H
#define PALERT_PALERT_H
...
/*        void calVars(const std::vector<KV> &kvs, int error) {
            ...
            palert::Builder<KeyType> asb(min_key_, max_key_, error*//*,alloc*//*);
        ...
        void setTreeLayout(const std::vector<KV> &kvs,size_t error=256) {
            ...
            palert::Builder<KeyType> asb(min_key, max_key, error/*,alloc*/);
            KeyType *keys = NULL;
            ...
            while (seg_message.size() + 1 > ROOT_SIZE) {
                ...
                int keyNum = 0;
                palert::Builder<KeyType> asb(min_key, seg_message.back().key, error);
                asb.AddKey(min_key);
                keys[keyNum++] = min_key;
